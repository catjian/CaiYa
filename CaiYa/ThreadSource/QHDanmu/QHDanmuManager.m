...
@interface QHDanmuManager () <QHDanmuContentViewDelegate>

@property (nonatomic, strong, readwrite) QHDanmuView *danmuView;
...
- (void)p_danmu:(NSTimeInterval)startTime {
    ...
    if (danmuInfos.count > 0)
    {
        ...
        if (YES)
        {
            void(^func)(QHDanmuContentView *danmuLabeV, NSUInteger idx, CGFloat offsetXY) = ^(QHDanmuContentView *danmuLabeV, NSUInteger idx, CGFloat offsetXY){
                if (idx != NSNotFound)
                {
                    [danmuLabeV setDanmuChannel:idx offset:offsetXY];
                    CGFloat time = [danmuLabeV startTime];
                    time = time - startTime;
                    [danmuLabeV animationDanmuItem:time];
                }
            };
            
            [danmuInfos enumerateObjectsUsingBlock:^(NSDictionary *obj, NSUInteger idx, BOOL *stop) {
                __block QHDanmuContentView *danmuView = [QHDanmuContentView createWithInfo:obj inView:weakSelf.danmuView];
                [danmuView setDelegate:self];
                if ([danmuView isMoveModeFadeOut])
                {
                    [weakSelf p_getFadeBestChannelView:danmuView completion:^(NSUInteger idx, CGFloat offsetY) {
                        func(danmuView, idx, offsetY);
                    }];
                }
                else
                {
                    [weakSelf p_getRollBestChannelView:danmuView completion:^(NSUInteger idx, CGFloat offsetX) {
                        func(danmuView, idx, offsetX);
                    }];
                }
            }];
        }
    ...
    {
        if (self.delegate && [self.delegate respondsToSelector:@selector(QHDanmuManamgerShowEnd)])
        {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(4 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [self.delegate QHDanmuManamgerShowEnd];
            });
        }
    }
}
...
- (void)p_getRollBestChannel:(QHDanmuLabel *)newDanmuL completion:(void(^)(NSUInteger idx, CGFloat offsetX))completion {
    ...
    if (bFind) {
        ...
        if ([obj isKindOfClass:[QHDanmuLabel class]]) {
            ...
            completion(index, x < 0 ?  arc4random_uniform(self.frame.size.height) : x);
        }else
            completion(index, arc4random_uniform(self.frame.size.height));
    }else
    {
...
//选择完全不会碰撞的航道
- (BOOL)p_last:(UIView *)lastDanmuL new:(UIView *)newDanmuL
{
    if ([newDanmuL isKindOfClass:[QHDanmuLabel class]] && [lastDanmuL isKindOfClass:[QHDanmuLabel class]])
    {
        CGFloat durationTime = [(QHDanmuLabel *)newDanmuL startTime] - [(QHDanmuLabel *)lastDanmuL startTime];
        if (durationTime > [(QHDanmuLabel *)newDanmuL animationDuartion]) {
            return YES;
        }
        CGFloat timeS = lastDanmuL.frame.size.width/[(QHDanmuLabel *)lastDanmuL speed];
        if (timeS >= durationTime) {
            return NO;
        }
        CGFloat timeE = [(QHDanmuLabel *)newDanmuL currentRightX]/[(QHDanmuLabel *)newDanmuL speed];
        if (timeE <= durationTime) {
            return NO;
        }
    }
    else if ([newDanmuL isKindOfClass:[QHDanmuContentView class]] && [lastDanmuL isKindOfClass:[QHDanmuContentView class]])
    {
        CGFloat durationTime = [(QHDanmuContentView *)newDanmuL startTime] - [(QHDanmuContentView *)lastDanmuL startTime];
        if (durationTime > [(QHDanmuContentView *)newDanmuL animationDuartion]) {
            return YES;
        }
        CGFloat timeS = lastDanmuL.frame.size.width/[(QHDanmuContentView *)lastDanmuL speed];
        if (timeS >= durationTime) {
            return NO;
        }
        CGFloat timeE = [(QHDanmuContentView *)newDanmuL currentRightX]/[(QHDanmuContentView *)newDanmuL speed];
        if (timeE <= durationTime) {
            return NO;
        }
    }
...
//选择在不超出缓冲区的且缓冲区最长的航道
- (NSUInteger)p_allChannelWithPosition:(DanmuPositionStruct)danmuPosition new:(UIView *)newDanmuL
{
...
- (void)p_getRollBestChannelView:(QHDanmuContentView *)newDanmuV completion:(void(^)(NSUInteger idx, CGFloat offsetX))completion
{
    DanmuPositionStruct danmuPosition;
    
    if (newDanmuV.isPositionMiddle)
    {
        danmuPosition = _middlePosition;
    }
    else if (newDanmuV.isPositionBottom)
    {
        danmuPosition = _downPosition;
    }
    else
    ...
    for (int i = (int)danmuPosition.start; i < danmuPosition.start + danmuPosition.length; i++)
    {
        id obj = [self.arRollChannelInfo objectAtIndex:i];
        index = i;
        if ([obj isKindOfClass:[QHDanmuContentView class]])
        {
            bFind = [self p_last:obj new:newDanmuV];
        }
        else
        {
            bFind = YES;
    ...
    {
        id obj = [self.arRollChannelInfo objectAtIndex:index];
        [self.arRollChannelInfo replaceObjectAtIndex:index withObject:newDanmuV];
        if ([obj isKindOfClass:[QHDanmuContentView class]])
        {
            CGFloat x = ((QHDanmuContentView *)obj).currentRightX;
            completion(index, x < 0 ?  arc4random_uniform(self.frame.size.height) : x);
        }
        else
            completion(index, arc4random_uniform(self.frame.size.height));
    ...
    else
    {
        ...
        {
            NSUInteger index = NSNotFound;
            index = [self p_allChannelWithPosition:danmuPosition new:newDanmuV];
            if (index != NSNotFound)
            {
                QHDanmuContentView *obj = [self.arRollChannelInfo objectAtIndex:index];
                [self.arRollChannelInfo replaceObjectAtIndex:index withObject:newDanmuV];
                CGFloat x = obj.currentRightX;
                completion(index, x < CHANNEL_SPACE ? CHANNEL_SPACE : x);
            }
...
- (void)p_getFadeBestChannelView:(QHDanmuContentView *)newDanmuV completion:(void(^)(NSUInteger idx, CGFloat offsetY))completion {
    DanmuPositionStruct danmuOnePosition;
    DanmuPositionStruct danmuTwoPosition;
    if (newDanmuV.isPositionMiddle) {
        danmuOnePosition = _middleFadeOnePosition;
    ...
    NSUInteger index = [self p_arDanmuContentView:ar1 position:danmuOnePosition new:newDanmuV];
    
    if (index != NSNotFound) {
        [ar1 replaceObjectAtIndex:index withObject:newDanmuV];
        completion(index, 0);
    }else {
        index = [self p_arDanmuContentView:ar2 position:danmuTwoPosition new:newDanmuV];
        
        if (index != NSNotFound) {
            [ar2 replaceObjectAtIndex:index withObject:newDanmuV];
            completion(index, newDanmuV.frame.size.height/2);
        }else {
            completion(NSNotFound, 0);
        }
    }
...
- (NSUInteger)p_arDanmuContentView:(NSMutableArray *)arDanmuLs position:(DanmuPositionStruct)danmuPosition new:(QHDanmuContentView *)newDanmuV {
    BOOL bFind = NO;
    NSUInteger index = danmuPosition.start;
    for (int i = (int)danmuPosition.start; i < danmuPosition.start + danmuPosition.length; i++) {
        id obj = [arDanmuLs objectAtIndex:i];
        index = i;
        if ([obj isKindOfClass:[QHDanmuContentView class]]) {
            QHDanmuContentView *lastDanmuV = obj;
            CGFloat durationTime = newDanmuV.startTime - lastDanmuV.startTime;
            bFind = (durationTime > (newDanmuV.animationDuartion - 1));
        }else {
            bFind = YES;
        }
...
- (void)resume:(NSTimeInterval)nowTime {
    ...
    for (id subview in self.danmuView.subviews) {
        ...
        if ([subview isKindOfClass:[QHDanmuContentView class]]) {
            [(QHDanmuContentView *)subview resume:nowTime];
        }
    }
}
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(QHDanmuManamgerChlickAction:)])
    {
        [self.delegate QHDanmuManamgerChlickAction:content];
    }
}
